...
public class DAOImpl {
	...
	public double getInsuranceAmt(String vehicleNo,String service) {
		...
		double netAmt = 0.0;
		try {
			String sql="select amount from insurancem where vehicleNo=? and service=?";
	...
	private double calcNetAmt(double amt,String service) {
		...
		String sql = "select servicePrice from servicem where serviceName = ?";
		try {
	...
	public void logInsurance(String vehicleNum,String service) {
		...
		String customerId="";
		double amount=0;
		String sql = "select customerId from vehiclem where vehicleNo=?";
		try {
			pstm = con.prepareStatement(sql);
